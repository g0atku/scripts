...
function CustomSprintScript()
	...
	RunService.Heartbeat:Connect(function(a)
		...
		if runanimation.IsPlaying and (not isChargingStam) and (not MyChar.IsSliding.Value and not MyHRP:FindFirstChild('IsTackling')) and (not MyHRP.Anchored) then
			...
			if os.time() - tacklecd >= 30 then
				if MyChar.Animate.idle.Animation1.AnimationId ~= "rbxassetid://12572855663" and not MyHRP:FindFirstChild("BodyVelocity") and (key.KeyCode == Enum.KeyCode.Q or key.KeyCode == Enum.KeyCode.ButtonX) and not MyHRP.Anchored then
					...
					if MyChar.Ragdoll.Value == false and tacklestam < stamina then
						...
						task.spawn(function()
							local IsTackling = Instance.new("StringValue")
							IsTackling.Name = "IsTackling"
							IsTackling.Parent = MyHRP
							task.wait(2.5)
							IsTackling:Destroy()
						end)
...
function CustomGKScript()
	...
	function BlockR()
		if MyChar:WaitForChild("GK").Value == false then
			if MyPlayer.Backpack:FindFirstChild("Serpent") and not isSaving then
				...
				if getgenv().IsVentilyator then
					task.spawn(function()
						local over = false
						task.delay(0.075, function()
							over = true
						end)
						while isSaving and not over do
							for i = 1,5 do
							...
							game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * 85 + Vector3.new(0, 80, 0), 'BlockR', true, false, true, true)
	...
	for i, v in ipairs(workspace:WaitForChild("Box"):GetChildren()) do
		UserInputService.InputBegan:Connect(function(key, a1)
			if not a1 then
				if (key.KeyCode == Enum.KeyCode[MyGui.Goalie:WaitForChild("Left").Value] or key.KeyCode == Enum.KeyCode.ButtonL1) and not isHoldingBall then
					...
					for _, part in pairs(partsinbox) do
						if part.Name == "HumanoidRootPart" then
							local HRP = part.Parent
							if HRP.Name == MyPlayer.Name and HRP then
								if v:FindFirstChild("Team") then
									if v:FindFirstChild("Team").Value == MyPlayer.Team.Name then
										BlockL()
									end
								else
									BlockL()
		...
		UserInputService.InputBegan:Connect(function(key, a1)
			if not a1 then
				if (key.KeyCode == Enum.KeyCode[MyGui.Goalie:WaitForChild("Right").Value] or key.KeyCode == Enum.KeyCode.ButtonR1) and not isHoldingBall then
					if MyPlayer.Backpack:FindFirstChild("Snatch") then
						isSnatch = true
					else
						isSnatch = false
		...
		UserInputService.InputBegan:Connect(function(key, a1)
			if not a1 then
				if (key.KeyCode == Enum.KeyCode[MyGui.Goalie:WaitForChild("Dive").Value] or key.KeyCode == Enum.KeyCode.ButtonX) and not isHoldingBall then
					if MyPlayer.Backpack:FindFirstChild("Acrobatic") then
						isAcrobatic = true
					else
						isAcrobatic = false
		...
		UserInputService.InputBegan:Connect(function(key, a1)
			if not a1 then
				if (key.KeyCode == Enum.KeyCode[MyGui.Goalie:WaitForChild("Block").Value] or key.KeyCode == Enum.KeyCode.ButtonB) and not isHoldingBall then
					if MyPlayer.Backpack:FindFirstChild("Snatch") then
						isSnatch = true
					else
						isSnatch = false
		...
		UserInputService.InputBegan:Connect(function(key, a1)
			if not a1 then
				if (key.KeyCode == Enum.KeyCode.R or key.KeyCode == Enum.KeyCode.DPadUp) and not isHoldingBall then
		...
		UserInputService.InputBegan:Connect(function(key, a1)
			if not a1 then
				if key.KeyCode == Enum.KeyCode[MyGui.Goalie:WaitForChild("PickUp").Value] and not isHoldingBall and not isSaving and MyChar:WaitForChild("GK").Value == true then
					local partsinbox = workspace:GetPartsInPart(v)
					for _, part in pairs(partsinbox) do
						if part.Name == "HumanoidRootPart" then
							local HRP = part.Parent
...
function TraitStack()
	if HMMLoaded then
		...
		if not table.find(nttable, 'Unbreakable') and getgenv().IsAutoM2 then
			table.insert(nttable, 'Unbreakable')
		end
		...
		if not newbp.Trait:FindFirstChild('Unbreakable') and getgenv().IsAutoM2 then
			local strait = game:GetService('ReplicatedStorage'):WaitForChild('Specs'):WaitForChild('Traits').Unbreakable:Clone()
			strait.Parent = newbp.Trait
		end
...
task.spawn(function()
	while task.wait() do
		if getgenv().AutoSlideTackle and MyChar.IsSliding.Value and GetClosestBall() and findAnimation(MyPlayer, '12994376714') then
			...
			repeat task.wait() until not MyChar.IsSliding.Value and not MyHRP:FindFirstChild('IsTackling')
		end
...
task.spawn(function()
	while task.wait() do
		if getgenv().IsAutoM2 and (GetClosestBall() and GetClosestBall():GetAttribute('player') ~= MyPlayer.Name) then			
			while getgenv().IsAutoM2 and not (GetClosestBall() and GetClosestBall():GetAttribute('player') == MyPlayer.Name) and not Library.Unloaded do
				if GetClosestBall() then
					kick(GetM2Direction())
				end
				task.wait()
...
if PlaceId ~= 12276235857 then
	...
	-- Configuration
	local beamWidth = math.sqrt(math.sqrt(1.9)) -- Width of the beam
	local segments = 10 -- Number of points on the trajectory
	local crossbars = {}
	...
	for i,crossbar in pairs(crossbars) do
		local goalhitbox = Instance.new('Part', workspace)
		goalhitbox.Name = 'goalhitbox'
		...
		goalhitbox.Transparency = 0.9
		goalhitbox.CFrame = crossbar.CFrame * CFrame.new(0,-6.9275,0)
		goalhitbox.Size = Vector3.new(0,17.655,42.825)
		goalhitbox.Touched:Connect(function(p)
			print(p)
			if string.find(p.Name, 'BeamPart') then
				print('GOAL!')
				local censor = Instance.new('Part', workspace)
				censor.Name = 'censor'
				censor.CanCollide = false
				censor.Anchored = true
				censor.Material = Enum.Material.Neon
				censor.Transparency = 0.9
				censor.CFrame = beamPart.CFrame
				censor.Size = Vector3.new(1.9,1.9,1.9)
	...
	Sections.GK:AddToggle('AutoDiveToggle', {
		Text = 'Auto Dive',
		Default = false,
		Tooltip = 'Automatically dives while being gk',
		Callback = function(Value)
			if Value then
				divecon = RunService.RenderStepped:Connect(function()
					-- Function to calculate the ball's landing point
					...
					-- Function to create or reuse beam parts
					function createOrUpdateBeam(beamFolder, startPos, endPos, i)
						local beamPart = beamFolder:FindFirstChild("BeamPart"..i)
						if not beamPart then
							beamPart = Instance.new("Part")
							beamPart.Name = "BeamPart"..i
							beamPart.Anchored = true
							beamPart.CanCollide = false
							beamPart.CanQuery = false
							beamPart.Transparency = 0
							beamPart.Size = Vector3.new(beamWidth, beamWidth, (startPos - endPos).Magnitude)
							beamPart.Color = getgenv().metacolor
							beamPart.Parent = beamFolder
						end
						local beamHl = beamPart:FindFirstChild('beamHl')
						if not beamHl then
							beamHl = Instance.new('Highlight')
							beamHl.Name = 'beamHl'
							beamHl.Parent = beamPart
							beamHl.Adornee = beamPart
							beamHl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
							beamHl.Enabled = true
							beamHl.FillColor = getgenv().metacolor
							beamHl.FillTransparency = 0
							beamHl.OutlineColor = getgenv().metacolor
							beamHl.OutlineTransparency = 0
						end
						beamPart.Color = getgenv().metacolor
						beamHl.FillColor = getgenv().metacolor
						beamHl.OutlineColor = getgenv().metacolor
					...
					-- Main script loop to manage beams
					for _, ball in pairs(workspace.BallFolder:GetChildren()) do
						if ball.Name == "Ball" then
							-- Check if the beam folder exists, if not create it
							local beamFolder = ball:FindFirstChild("TrajectoryBeams")
							if not beamFolder then
								beamFolder = Instance.new("Folder")
								beamFolder.Name = "TrajectoryBeams"
								beamFolder.Parent = ball
							end

							-- Calculate trajectory positions
							local trajectoryPositions = calculateTrajectory(ball, segments)

							-- Create or update beams for the trajectory
							for i = 1, #trajectoryPositions - 1 do
								local startPos = trajectoryPositions[i]
								local endPos = trajectoryPositions[i + 1]
								createOrUpdateBeam(beamFolder, startPos, endPos, i)
							end

							-- Remove extra beams if needed
							for i = #trajectoryPositions, #beamFolder:GetChildren() do
								beamFolder:FindFirstChild("BeamPart"..i):Destroy()
							end
						end
					end
					if Library.Unloaded then
						...
						for _, ball in pairs(workspace.BallFolder:GetChildren()) do
							if ball.Name == "Ball" then
								if ball:FindFirstChild("TrajectoryBeams") then
									ball:WaitForChild("TrajectoryBeams"):Destroy()
								end
							end
						end
					end
	...
	if Discord and (Discord == '<@405342167515529226>' or Discord == '<@936346010270203956>') then
		Sections.GK:AddToggle('VentilyatorToggle', {
			Text = 'Ventilyator',
			Default = false,
			Tooltip = 'Enables Ventilyator',
			Callback = function(Value)
				getgenv().IsVentilyator = Value
			end
		})
	end
