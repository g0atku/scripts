...
getgenv().GKHBE = false
getgenv().IsGKHBE = 0
getgenv().IsFDive = false
getgenv().FlickDirection = 'Char Movement'
getgenv().StaminaGain = 1
getgenv().SlideTackleMulti = 1
getgenv().GKDiveMulti = 1
getgenv().GKDiveClearMulti = 1
getgenv().GKDiveCDMulti = 1
getgenv().GKDiveRagdollMulti = 1
...
function CustomSprintScript()
	...
	function checksprint(a1, a2, a3)
		if MyGui.Menu.NewPopup.GUIS.SettingsGUI.Sprint.Value.Value then
			handle(a1, a2, a3)
		end
	end
	...
	RunService.Heartbeat:Connect(function(a)
		...
		local getstam = MyChar:GetAttribute("MAXSTAMINA") or 100
		if workspace:WaitForChild("TrainingMode").Value or (MyChar:FindFirstChild("Lroke") and MyChar:WaitForChild("Lroke").Value) then
			stamina = getstam
		end
	...
	UserInputService.InputBegan:Connect(function(key, a1)
		if (key.KeyCode == Enum.KeyCode.V or key.KeyCode == Enum.KeyCode.ButtonY) and MyChar and not MyChar:FindFirstChildOfClass("Tool") and not isChargingStam and not MyChar.Ragdoll.Value then
			isChargingStam = true
	...
	function funccooldown(cd)
		...
		if not MyGui.GeneralGUI.cooldowns.Tacklecd.TextLabel.Visible then
		...
		elseif isTackle then
			if os.time() - tacklecd >= 30 then
				if MyChar.Animate.idle.Animation1.AnimationId ~= "rbxassetid://12572855663" and not MyHRP:FindFirstChild("BodyVelocity") and (key.KeyCode == Enum.KeyCode.Q or key.KeyCode == Enum.KeyCode.ButtonX) and not MyHRP.Anchored then
					...
					if not MyChar.Ragdoll.Value and tacklestam < stamina then
						...
						if isSurf then
							TackleVel.Velocity = TackleVel.Velocity * (1.3+(1-getgenv().SlideTackleMulti))
						end
...
function CustomGKScript()	
	...
	function cooldownbuff(cd)
		if workspace:FindFirstChild("TrainingMode") then
			cd = (workspace.TrainingMode.Value and 0) or cd
		end
	...
	function cooldownmove(cd)
		...
		if not MyGui.GeneralGUI.cooldowns.Weaponcd.TextLabel.Visible then
	...
	function BlockR(extend)
		...
		if not MyChar:WaitForChild("GK").Value then
			if MyPlayer.Backpack:FindFirstChild("Serpent") and not isSaving then
				...
				vel.Parent = MyHRP
				game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockR", false, false, true, false)
				local extended = false
				if extend then
					extended = true
					game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", false, false, false, false)
					game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockR", false, false, false, false)
				end
				if getgenv().IsGKHBDuration then
					task.spawn(function()
						for i = 1,3 do
							task.wait(0.3)
							if GetClosestBall() and GetClosestBall():GetAttribute('kickiframes') ~= MyPlayer.Name then
								if extended then
									extended = false
									game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", false, false, false, false)
									game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockL", false, false, false, false)
									game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockL", false, false, false, false)
									game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockR", false, false, false, false)
								else
									game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", false, false, false, false)
				...
				task.wait(1-(1*(getgenv().GKDiveCDMulti-1)))
				isSaving = false
				return
			end
		elseif not isSaving then
			...
			if MyPlayer.Backpack.Trait:FindFirstChild("Diver") then
				direction = MyHRP.CFrame.RightVector * (200 * getgenv().GKDiveMulti)
			end
			blockfanimation:Stop()
			blockranimation:Play()
			if isAcrobatic and not gagamarucd then
				game:GetService("ReplicatedStorage"):WaitForChild("Ragdoll"):FireServer(0.6, 0.2-(0.2*(getgenv().GKDiveRagdollMulti-1)))
				task.spawn(function()
					task.wait(0.7-(0.7*(getgenv().GKDiveRagdollMulti-1)))
					blockranimation:Stop()
				end)
			else
				game:GetService("ReplicatedStorage"):WaitForChild("Ragdoll"):FireServer(0.6, 0.6-(0.6*(getgenv().GKDiveRagdollMulti-1)))
				task.spawn(function()
					task.wait(1.1-(1.1*(getgenv().GKDiveRagdollMulti-1)))
					blockranimation:Stop()
				end)
			end
			vel.Velocity = direction
			vel.Parent = MyHRP
			game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockR", isSnatch, false, true, isClaw)
			local extended = false
			if extend then
				extended = true
				game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", isSnatch, false, false, isClaw)
				game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockR", isSnatch, false, false, isClaw)
			end
			if getgenv().IsGKHBDuration then
				task.spawn(function()
					for i = 1,3 do
						...
						if GetClosestBall() and GetClosestBall():GetAttribute('kickiframes') ~= MyPlayer.Name then
							if extended then
								extended = false
								game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", isSnatch, false, false, isClaw)
								game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockL", isSnatch, false, false, isClaw)
								game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockL", isSnatch, false, false, isClaw)
								game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockR", isSnatch, false, false, isClaw)
							else
								game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", isSnatch, false, false, isClaw)
			...
			task.wait(0.5-(0.5*(getgenv().GKDiveCDMulti-1)))
			isSaving = false
		end
	end
	function BlockL(extend)
		if isHoldingBall or not InBox() then return end
		if MyPlayer.Backpack:FindFirstChild("Snatch") then
			isSnatch = true
		else
		...
		if not MyChar:WaitForChild("GK").Value then
			if MyPlayer.Backpack:FindFirstChild("Serpent") and not isSaving then
				...
				vel.Velocity = -MyHRP.CFrame.RightVector * (130 * getgenv().GKDiveMulti)
				vel.Parent = MyHRP
				game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockL", false, false, true, false)
				local extended = false
				if extend then
					extended = true
					game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", false, false, false, false)
		...
		elseif not isSaving then
			...
			vel.Velocity = direction
			vel.Parent = MyHRP
			game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockL", isSnatch, false, true, isClaw)
			local extended = false
			if extend then
				extended = true
				game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", isSnatch, false, false, isClaw)
	...
	function handleStealEvent()
		...
		if MyChar:WaitForChild("GK").Value then
			if not isSaving then
				...
				blockfanimation:Play()
				MyChar.IsBlocking.Value = true
				game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), hb, isSnatch, false, false, isClaw)
				if getgenv().IsGKHBDuration then
					task.spawn(function()
						for i = 1,3 do
							task.wait(0.3)
							if GetClosestBall() and GetClosestBall():GetAttribute('kickiframes') ~= MyPlayer.Name then
								...
								game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), 'Gagamaru', isSnatch, false, false, isClaw)
								if hb == 'Handle' then
									game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), 'Handle', isSnatch, false, false, isClaw)
								end
								if getgenv().IsGKHBE > 0 then
									for i = 1,getgenv().IsGKHBE do
										game:GetService("ReplicatedStorage").GK.jumped:FireServer()
	...
	function sdive(dir)
		...
		if not MyChar:WaitForChild("GK").Value then
			if MyPlayer.Backpack:FindFirstChild("Serpent") and not isSaving then
				...
				if dir == 'L' then
					sblocklanimation:Play()
					vel.MaxForce = Vector3.new(1, 0, 1) * 19000
					vel.Velocity = -MyHRP.CFrame.RightVector * 45
					vel.Parent = MyHRP
					game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockR", false, false, false, false)
					game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "BlockL", false, false, true, false)
					if getgenv().IsGKHBDuration then
		...
		elseif not isSaving then
			...
			if dir == 'L' then
				blockfanimation:Play()
				vel.MaxForce = Vector3.new(1, 0, 1) * 19000
				local direction = -MyHRP.CFrame.RightVector * 45
				blockfanimation:Stop()
				blocklanimation:Play()
				if isAcrobatic and not gagamarucd then
					game:GetService("ReplicatedStorage"):WaitForChild("Ragdoll"):FireServer(0.6, 0.2-(0.2*(getgenv().GKDiveRagdollMulti-1)))
	...
	function gagamaruactive()
		...
		if MyChar:WaitForChild("GK").Value then
			if not isSaving and not gagamarucd and isAcrobatic then
				...
				isClaw = false
				game.ReplicatedStorage:WaitForChild("GK"):WaitForChild("Goalie"):FireServer(MyHRP.CFrame.lookVector * (85 * getgenv().GKDiveClearMulti) + Vector3.new(0, 80, 0), "Gagamaru", isSnatch, isGagamaru, false, isClaw)
				if getgenv().IsGKHBDuration then
					task.spawn(function()
						for i = 1,3 do
							task.wait(0.3)
	...
	function divef()
		...
		if MyChar:WaitForChild("GK").Value then
			if not isSaving then
				...
				if MyPlayer.Backpack.Trait:FindFirstChild("Diver") then
					direction = MyHRP.CFrame.LookVector * (140 * getgenv().GKDiveMulti)
				end
	...
	function PickUpBall()
		if InBox() and not isHoldingBall and not isSaving and MyChar:WaitForChild("GK").Value then
			...
			task.wait(1.5-(1.5*(getgenv().GKDiveCDMulti-1)))
			isSaving = false
		end
	end
	UserInputService.InputBegan:Connect(function(key, a1)
	...
	Sections.GK:AddSlider('GKDiveClearSlider', {
		Text = 'GK Dive Clear Multi',
		Default = 1,
		Min = 1,
		Max = 2,
		Rounding = 2,
		Compact = false,
		Callback = function(Multi)
			getgenv().GKDiveClearMulti = Multi
		end
	})
	Sections.GK:AddSlider('GKDiveCDSlider', {
		Text = 'GK Dive CD Multi',
		Default = 1,
		Min = 1,
		Max = 2,
		Rounding = 2,
		Compact = false,
		Callback = function(Multi)
			getgenv().GKDiveCDMulti = Multi
		end
	})
	Sections.GK:AddSlider('GKDiveRagdollSlider', {
		Text = 'GK Dive Ragdoll Multi',
		Default = 1,
		Min = 1,
		Max = 2,
		Rounding = 2,
		Compact = false,
		Callback = function(Multi)
			getgenv().GKDiveRagdollMulti = Multi
		end
	})
	...
	Sections.GK:AddToggle('UseFDiveToggle', {
		Text = 'Use F dive',
		Default = false,
		Tooltip = 'Will use F dive while using auto dive',
		Callback = function(Value)
			getgenv().IsFDive = Value
		end
	})
	local segments = 10
	local crossbars = {}
					...
					task.spawn(function()
						for _, ball in pairs(workspace.BallFolder:GetChildren()) do
							if ball.Name == "Ball" and ball.AssemblyLinearVelocity.Magnitude > 0 then
								local trajectoryPositions = calculateTrajectory(ball, segments)[1]
								local trajectoryTimings = calculateTrajectory(ball, segments)[2]
								for i = 1, #trajectoryPositions - 1 do
									local startPos = trajectoryPositions[i]
									local endPos = trajectoryPositions[i + 1]
									local distance = (endPos-startPos).Magnitude
									local startTime = trajectoryTimings[i]
									local endTime = trajectoryTimings[i + 1]
									local timeStep = startTime-endTime
									...
									if rayResult then
										if string.find(rayResult.Instance.Name, 'savehitbox') then
											...
											local p = (endPos-rayResult.Position).Magnitude / distance
											local PingDelay = (game:GetService('Stats').Network.ServerStatsItem['Data Ping']:GetValue()+10) / 1000
											local BallDelay = timeStep * p
											local hitTime = endTime + BallDelay
											if hitTime - PingDelay <= (PingDelay + BallDelay) + 0.3 then						
												if MyHRP.CFrame:ToObjectSpace(workspace:WaitForChild('goalsencor').CFrame).X < -6.375 then
													if rayResult.Position.Y >= workspace:WaitForChild('GoalHitboxes').goalhitbox1.CFrame.Y-1 and not fatiguecd and not isSaving and not isHoldingBall then
														MyHum:ChangeState(Enum.HumanoidStateType.Jumping)
													end
													if MyHRP.CFrame:ToObjectSpace(workspace:WaitForChild('goalsencor').CFrame).X < -13.5 then
														BlockL(true)
													else
														BlockL(false)
													end
												elseif MyHRP.CFrame:ToObjectSpace(workspace:WaitForChild('goalsencor').CFrame).X > 6.375 then
													...
													if MyChar:WaitForChild("GK").Value then
														if getgenv().IsFDive then
															if rayResult.Position.Y >= workspace:WaitForChild('GoalHitboxes').goalhitbox1.CFrame.Y and not fatiguecd and not isSaving and not isHoldingBall then
																MyHum:ChangeState(Enum.HumanoidStateType.Jumping)
															end
															handleStealEvent()
														else
															...
															if MyHRP.CFrame:ToObjectSpace(workspace:WaitForChild('goalsencor').CFrame).X >= 0 then
																sdive('R')
															else
																sdive('L')
															end
	...
	task.spawn(function()
		while task.wait() do
			...
			if Options.AutoDiveToggleKeyBind.Mode == 'Hold' then
				if Options.AutoDiveToggleKeyBind:GetState() then
					getgenv().IsAutoDive = true
				end
			elseif Options.AutoDiveToggleKeyBind.Mode == 'Always' then
				getgenv().IsAutoDive = true
			end
		end
	end)
...
if PlaceId ~= 12276235857 then
	if MyChar.Movement:FindFirstChild('Sprint') then
		repeat task.wait() until (MyHum.MoveDirection.Magnitude > 0 and MyHum.WalkSpeed <= 18)
	...
	if PlaceId == 13705109069 or PlaceId == 17079303871 then
		...
		task.spawn(function()
			while true do
				repeat task.wait(getgenv().FlowGain) until not MyChar:WaitForChild("FlowValue").Value
				if not workspace.TrainingMode.Value then
...
if HMMLoaded or (orgWeapon.Name == 'Formless' or orgWeapon.Name == 'Acrobatic') then
...
function dribble()
	if MyHum.FloorMaterial == Enum.Material.Air and not chestbumpcd then
		...
		task.spawn(function() 
			repeat task.wait() until MyHum.FloorMaterial ~= Enum.Material.Air
			chestbumpcd = false
		end)
	elseif not dribblecd then
		...
		if Options.DribbleKeyBind.Mode == 'Toggle' then
			if getgenv().DribbleDelay < 0.1 then
	...
	while task.wait() do
		if getgenv().IsAutoM2 and (GetClosestBall() and GetClosestBall():GetAttribute('player') ~= MyPlayer.Name) and not isSaving and not isHoldingBall then
			while getgenv().IsAutoM2 and not (GetClosestBall() and GetClosestBall():GetAttribute('player') == MyPlayer.Name) and not isSaving and not isHoldingBall and not Library.Unloaded do
				if InAir() and MyGui.GeneralGUI.CurrentPower.PWR.Value == 0 then return end
				if not MyHRP:FindFirstChild('IsTackling') then
					local IsTackling = Instance.new("StringValue", MyHRP)
					IsTackling.Name = "IsTackling"
				end
			...
			if (GetClosestBall() and GetClosestBall():GetAttribute('player') == MyPlayer.Name and GetClosestBall():GetAttribute('breakkick')) then
				task.delay(0.2, function() 
					MyGui.GeneralGUI.CurrentPower.PWR.Value = 0
				end)
				if MyGui.GeneralGUI.CurrentPower.PWR.Value > 0 then
			...
			task.spawn(function()
				repeat task.wait() until not kickcd
				task.wait(0.185)
				if MyHRP:FindFirstChild('IsTackling') and not kickcd then
					MyHRP:WaitForChild('IsTackling'):Destroy()
				end
			end)
		end
...
function weaponcooldownbuff(cd)
    local getcd
    if workspace:FindFirstChild("TrainingMode") then
        getcd = (workspace.TrainingMode.Value and 0) or cd
    else
        getcd = cd
    end
    if MyPlayer.Backpack.Trait:FindFirstChild("Consistent") then
...
function Formless()
	if MyPlayer.Backpack:FindFirstChild("Formless") and not MyGui.GeneralGUI.cooldowns.Weaponcd.TextLabel.Visible then
		if getgenv().IsAutoFormless and GetClosestBall() then
			local pos = MyHRP.CFrame.p-Vector3.new(CurrentCamera.CFrame.LookVector.X,0,CurrentCamera.CFrame.LookVector.Z)
...
if PlaceId ~= 12276235857 then
		...
		Callback = function(Value)
			if Value then
				mvcon = RunService.RenderStepped:Connect(function()
					...
					if Library.Unloaded then
						if mvcon then mvcon:Disconnect() end
						for _, ball in pairs(workspace.BallFolder:GetChildren()) do
							if ball.Name == "Ball" then
								if ball:FindFirstChild('MetavisionLandingPart') then
	...
	Sections.Dribbling:AddDropdown('FlickDirectionDropdown', {
		Values = {'Char Movement','Char Direction','Camera Direction'},
		Default = 1,
		Multi = false,
		Text = 'Flick Direction',
		Tooltip = 'Flicks the ball to the chosen direction',
		Callback = function(Value)
			getgenv().FlickDirection = Value		
		end
	})
	Sections.Dribbling:AddToggle('TurnCharToggle', {
		Text = 'Turn Character',
		Default = false,
		Tooltip = 'Turns Character when flicking',
		Callback = function(Value)
			getgenv().IsTurnCharFlick = Value
		end
	})
	...
	Sections.Dribbling:AddLabel('Flick'):AddKeyPicker('FlickToggleKeyBind', {
		...
		NoUI = true,
		Callback = function(Value)
			if getgenv().IsAutoFlick and GetClosestBall() and not kickcd then
				local direction
				local turndirection
				if getgenv().FlickDirection == 'Char Movement' then
					if ((MyHRP.CFrame.p+MyHum.MoveDirection)-GetClosestBall().CFrame.p).Magnitude > (MyHRP.CFrame.p-GetClosestBall().CFrame.p).Magnitude then
						...
						turndirection = Vector3.new(direction.X,MyHRP.CFrame.Y,direction.Z)
					else
						direction = Vector3.new(MyHRP.CFrame.X,MyHRP.CFrame.Y+FlickHeight,MyHRP.CFrame.Z)
						local dir = (MyHRP.CFrame.p-GetClosestBall().CFrame.p).Unit
						turndirection = MyHRP.CFrame.p + dir
					end
				elseif getgenv().FlickDirection == 'Char Direction' then
					local lv = Vector3.new(MyHRP.CFrame.LookVector.X,0,MyHRP.CFrame.LookVector.Z)
					if ((MyHRP.CFrame.p+lv)-GetClosestBall().CFrame.p).Magnitude > (MyHRP.CFrame.p-GetClosestBall().CFrame.p).Magnitude then
						direction = Vector3.new(MyHRP.CFrame.X,MyHRP.CFrame.Y+FlickHeight,MyHRP.CFrame.Z) + (lv*FlickThrough)
						turndirection = Vector3.new(direction.X,MyHRP.CFrame.Y,direction.Z)
					else
						direction = Vector3.new(MyHRP.CFrame.X,MyHRP.CFrame.Y+FlickHeight,MyHRP.CFrame.Z)
						local dir = (MyHRP.CFrame.p-GetClosestBall().CFrame.p).Unit
				...
				elseif getgenv().FlickDirection == 'Camera Direction' then
					local lv = Vector3.new(CurrentCamera.CFrame.LookVector.X,0,CurrentCamera.CFrame.LookVector.Z)
					if ((MyHRP.CFrame.p+lv)-GetClosestBall().CFrame.p).Magnitude > (MyHRP.CFrame.p-GetClosestBall().CFrame.p).Magnitude then
						direction = Vector3.new(MyHRP.CFrame.X,MyHRP.CFrame.Y+FlickHeight,MyHRP.CFrame.Z) + (lv*FlickThrough)
						turndirection = Vector3.new(direction.X,MyHRP.CFrame.Y,direction.Z)
					else
				...
				if getgenv().IsTurnCharFlick then
					task.spawn(function()
						MyHum.AutoRotate = false
						MyHRP.CFrame = CFrame.new(MyHRP.CFrame.p, turndirection)
						task.wait(0.2)
						MyHum.AutoRotate = true
					end)
				end
...
if PlaceId == 12276235857 then
	...
	game:GetService("ReplicatedStorage").SendCode:FireServer('SEASON2PROLEAGUE')
	game:GetService("ReplicatedStorage").SendCode:FireServer('40KLIKES')
